...
class ManageStoreCommand extends Command{
    ...
    private function createSupervisorConfig($storeId){
        ...
        command=php /var/www/easylink/artisan queue:work redis --queue=store_{$storeId}_order_transfer,store_{$storeId}_order_update,store_{$storeId}_inventory_update,store_{$storeId}_customer_transfer,store_{$storeId}_product_update  --sleep=3 --tries=3
        autostart=true
        autorestart=true
        user=www-data
        numprocs=3
