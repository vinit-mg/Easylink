...
class ManageStoreCommand extends Command{
    ...
    private function createSupervisorConfig($storeId){
        ...
        command=php /path-to-your-app/artisan queue:work redis --queue=store_{$storeId}_order_transfer,store_{$storeId}_order_update --sleep=3 --tries=3
        autostart=true
        autorestart=true
        user=www-data
        numprocs=3
        redirect_stderr=true
        stdout_logfile=/path-to-logs/store_{$storeId}.log
        ";
        ...
        exec("supervisorctl start store_{$storeId}:*");
    ...
    private function removeSupervisorConfig($storeId)
    {
        ...
        if (file_exists($configPath)) {
            ...
            exec('supervisorctl reread');
            exec('supervisorctl update');
            exec("supervisorctl stop store_{$storeId}:*");
        }
