...
class StoreController extends Controller{
    ...
    public function savePermissions(Request $request, Customers $customer, Stores $store){
        ...
        // Delete existing permissions for the store
        StorePermission::where('store_id', $store->id)->delete();

        // Define permission keys
        $permissionKeys = [
            'store_order_permissions',
            'store_inventory_permissions',
            'store_customer_permissions'
        ];

        foreach ($permissionKeys as $key) {

            if (!isset($formFields[$key]) || !is_array($formFields[$key])) {
                continue;
            }

            $storePermissions = collect($formFields[$key]);

            // Fetch package features in bulk to reduce queries
            $packageFeatures = PackageFeatures::whereIn('id', $storePermissions)
                ->where('include_scheduler', true)
                ->get()
                ->keyBy('id');

            $storePermissions->each(function ($storePermission) use ($store, $packageFeatures) {
                // Check if package feature includes a scheduler
                if ($packageFeatures->has($storePermission)) {
                    Schedulers::updateOrCreate(
                        [
                            'store_id' => $store->id,
                            'package_feature_id' => $storePermission, // Fixed spacing issue
                        ],
                        [
                            'store_id' => $store->id,
                            'package_feature_id' => $storePermission,
                            'IsActive' => false,
                        ]
                    );
                }
                ...
                StorePermission::create([
                    "store_id" => $store->id,
                    "package_featurte_id" => $storePermission
                ]);
            });
        }
