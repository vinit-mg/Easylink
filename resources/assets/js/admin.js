...
$('#fatchAcroCustomer').on('click', function (e) {
  ...
  $.ajax({
      type:'POST',
      url:route('admin.stores.getCustomerName'),
      ...
      success:function(data){
        if (typeof data.body != 'undefined' && data.body !== '') {
            ...
            bodyData.forEach(Customer => {
                let option = document.createElement("option");
                option.value = Customer.code; // or any unique identifier
                option.textContent = Customer.name; // Assuming the Customer object has a name field
                dropdown.append(option);
            });
            swal({
                title: 'Connected',
                text: 'Customers fatched successfully',
                icon: 'success',
      ...
      success:function(data){
        ...
        if (typeof data.body != 'undefined' && data.body !== '') {
            data.body.forEach(version => {
                ...
                option.textContent = version.displayName; // Assuming the Customer object has a name field
                dropdown.append(option);
            });
            swal({
                title: 'Connected',
