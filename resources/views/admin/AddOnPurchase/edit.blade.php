            ...
            <div class="card">
              <div class="card-body">
                  <div class="align-items-center card-title d-lg-flex d-md-block justify-content-between">
                    <h5>{{__('Update AddOns Purchase')}}</h5>
                  ...
                  <form class="row g-3" method="POST" action="{{ route('admin.addonpurchase.update', $addonpurchase->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        ...
                        <select name="Customer_subscription_id" id="Customer_subscription_id" class="form-select">
                            ...
                            @foreach ($CustomerSubscriptions as $CustomerSubscription)
                                <option value="{{$CustomerSubscription->id}}" {{$addonpurchase->Customer_subscription_id == $CustomerSubscription->id ? 'selected' : ''}}>{{$CustomerSubscription->package->name}}</option>
                            @endforeach
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        ...
                        <select name="add_on_id" id="add_on_id" class="form-select">
                            ...
                            @foreach ($AddOns as $AddOn)
                                <option value="{{$AddOn->id}}" {{$addonpurchase->add_on_id == $AddOn->id ? 'selected' : ''}}>{{$AddOn->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <label for="type" class="form-label">{{__('Quantity')}}</label>
                        <input type="number" id="quantity" name="quantity" class="form-control" value="{{$addonpurchase->quantity}}">
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <label for="addon_price" class="form-label">{{__('AddOn price')}}</label>
                        <div class="input-group mb-3">
                            <span class="input-group-text">Kr.</span>
                            <input type="number" name="addon_price" class="form-control" min="0" step="1.00" value="{{$addonpurchase->addon_price}}">
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        ...
                        <div class="input-group mb-3">
                            <span class="input-group-text">Kr.</span>
                            <input type="number" name="total_price" id="total_price" class="form-control" min="0" step="1.00" value="{{$addonpurchase->total_price}}">
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <label for="purchased_at" class="form-label">{{__('Purchased date')}}</label>
                        <input type="datetime-local" class="form-control" name="purchased_at" id="purchased_at" value="{{$addonpurchase->purchased_at}}">
                    </div>
